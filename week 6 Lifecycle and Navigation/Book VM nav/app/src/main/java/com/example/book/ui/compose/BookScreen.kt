...
fun BookScreen(onNavigateToBookDetail:(bookName: String, author: String) -> Unit) {
        ...
        content = {
            ...
            )
            {
                items(viewModel.bookList, key={book -> book.id}) { book ->
                    BookItem(book, context, {viewModel.delete(it)}, onItemClick = onNavigateToBookDetail)
                }
            }
        }
    )
...
@Composable
fun BookItem(item: Book, context: Context, deleteBook: (Book) -> Unit, onItemClick: (bookName: String, author: String) -> Unit) {
    ...
    Card(
        ...
        modifier = Modifier
            ...
            .combinedClickable(
                onClick = {
                    onItemClick(item.bookName, item.author)},
//                    Toast
//                        .makeText(
//                            context,
